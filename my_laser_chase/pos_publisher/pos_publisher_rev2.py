...
# number of grid cells in the x,y,z directions of grid coordinate system
(x_size, y_size, z_size) = (70, 40, 50)

# length per grid cell in millimeter
lenPerCell = 5

# position of grid origin in robot coordinate system in millimeter.
...
# transformation matrix from grid to robot
g_T_r = inv(r_T_g)

# distance in mm between camera and laser dots
depth = 345

# camera matrix obtained using millimeter measurements
...
def createWorkspaceGrid(x_size, y_size, z_size, r_T_g, lenPerCell):
    ...
    changes from grid coordinate system to robot coordinate system,
    call kinematics.solve() to determine if the cell is reachable
    
    # ix,iy,iz are the x,y,z-indices of the grid. Note that the convention is grid[z,y,x]
...
def getLaserContour(tilted_frame, tiltedCorners, headonCorners):
    ...
    contour is an element of the list "contours". contour itself is a list of coordinates that surrounds the laser dot.
        -It is in the coordinate system of the image plane of the virtual head-on camera
    ...
    cv2.drawContours(frame,contours,-1,(0,255,0),1) #directly modifies "frame"
...
def getLaserPosition(contour):
    """
    Argument:
    contour is an element of the list "contours". contour itself is a list of coordinates that surrounds the laser dot.
        -It is from the perspective of the virtual head-on camera
    
    Return:
    r_vec is the 4x1 homogenous vector from robot to laser dot in the robot coordinate system
    """
    
    # contour centroid (cc) in pixels in the coordinate system of the virtual head-on camaer
    M = cv2.moments(contour)
    cc_x = int(M['m10']/M['m00'])
    cc_y = int(M['m01']/M['m00'])
    
    # create homogeneous image point using centroid center.
    imgPt = np.array([[cc_x],
                      [cc_y],
                      [1 ]])
    
    # vector from virtual head-on camera to laser point. k_inv is inverse of the intrinsic camaera matrix
    ...
    # transformation matrix from robt to camera.
        ...
        # so I shine the laser at the crosshair of the board when running this python file. The crosshair intersects with the y-axis of the robot coordinate system.
        # r_vec is printed out in the console.
    ...
    # vector from robot to laser dot in the robot coordinate system
...
def getRobotPosition(r_vec, x_size, y_size):
    ...
    Argument:
    r_vec is the 4x1 homogenous vector from robot to laser dot in the robot coordinate system
    x_size and y_size are the sizes of the grid in the grid coordinate system
    Return:
    ...
    pos = Pose()
    # g_vec is the vector pointing from grid origin to the laser dot in the grid coordinate system
    g_vec = g_T_r.dot(r_vec)
    gx = g_vec[0,0]
    gy = g_vec[1,0]
    
    # get indices in the grid coordinate system
...
while not rospy.is_shutdown():
    ...
    if contourGood == True:
        r_vec = getLaserPosition(contour)
